...
public class HatchAutoChamps extends AutoModeBase {
    ...
    protected void routine() throws AutoModeEndedException 
    {
        ...
        double   fastSpeed = 72;
        double    superFastSpeed = 120;
        double    medSpeed = 48;
        double   slowSpeed = 24;
        double visionSpeed = 36;

        double     accelTime = 1.0;     // time to accelerate to full speed
        // double     superFastAccelTime = 0.25;     // time to accelerate to full speed
        double     superFastAccelTime = 1.0;     // time to accelerate to full speed
        ...
        PathSegment.Options   SuperFastOptions = new PathSegment.Options( superFastSpeed,   superFastSpeed/superFastAccelTime, superFastSpeed/lookaheadTime, false);
        ...
        {
            Vector2d level1Pos = target1StartPos.add(new Vector2d(48, 0));
            firstTargetPathB1 = new Path();
            firstTargetPathB1.add(new Waypoint(target1StartPos,      SuperFastOptions));
            firstTargetPathB1.add(new Waypoint(level1Pos, SuperFastOptions));
    
            firstTargetPathB2 = new Path();
            firstTargetPathB2.add(new Waypoint(level1Pos,      fastOptions));
            firstTargetPathB2.add(new Waypoint(target1BackupTurnPos, fastOptions));
        ...
        if (target1 != FieldDimensions.TargetPositionEnum.CARGO_FRONT)
        {
            ...
            runAction(new PathFollowerAction(firstTargetPathB2));
        }
